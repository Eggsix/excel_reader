...
(function() {
  var BOOLTEXT, BOOLVALS, assign, convert, convertValue, convertValueList, excel, fs, isArray, parseKeyName, path, processFile, transpose, write,
  ...
  convert = function(data, isColOriented) {
    var index, item, j, k, keys, len, len1, result, row, rows, value;
    if (isColOriented == null) {
      isColOriented = false;
    }
    if (isColOriented) {
      data = transpose(data);
  ...
  processFile = function(src, dst, isColOriented, callback) {
    if (isColOriented == null) {
      isColOriented = false;
    }
    if (callback == null) {
    ...
    if (!fs.existsSync(src)) {
      ...
      return excel(src, function(err, data) {
        var result;
        if (err) {
